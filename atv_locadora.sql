...
-- 9) HAVING: clientes que fizeram mais de 1 locação
...
-- 10) ORDER BY: filmes mais alugados
...
ORDER BY vezes_alugado DESC, f.titulo;

-- 11) JOIN múltiplo: elenco de cada filme
...
-- 12) JOIN múltiplo: detalhes completos da locação
SELECT 
    l.id_locacao,
    c.nome AS cliente,
    ...
    f.titulo AS filme,
    m.tipo_midia,
    il.data_prevista_devolucao,
    il.data_devolucao,
    il.multa
FROM locacoes l
INNER JOIN clientes c ON l.id_cliente = c.id_cliente
INNER JOIN funcionarios fun ON l.id_funcionario = fun.id_funcionario
...
ORDER BY l.id_locacao, f.titulo;

-- 13) Condições compostas (AND + OR)
...
-- 14) Identificar mídias que nunca foram alugadas
...
-- 15) Controle de estoque por mídia
SELECT 
    f.titulo,
    SUM(CASE WHEN m.status_midia = 'Disponível' THEN 1 ELSE 0 END) AS disponiveis,
    SUM(CASE WHEN m.status_midia = 'Alugada' THEN 1 ELSE 0 END) AS alugadas,
    SUM(CASE WHEN m.status_midia = 'Manutenção' THEN 1 ELSE 0 END) AS manutencao,
